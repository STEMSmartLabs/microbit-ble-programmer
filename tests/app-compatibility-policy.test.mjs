...
test('classifies only the terminal bonded 0004 access failure', () => {
  ...
  assert.match(
    patched,
    /Could not enable bonded DFU indications after the one allowed reconnect/,
  );
  assert.match(
    patched,
    /Could not enable bonded DFU indications on 0004:/,
  );
});

test('shows one-time USB base-program guidance for incompatible Bluetooth setup', () => {
...
test('retains the v2.4.12 recovery and proven transfer policy', () => {
